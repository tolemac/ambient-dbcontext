 ...
 */
 /* 
  * Ported to Net Standard by Javier Ros (aka tolemac or Jros)
  */
...
namespace Jros.AmbientDbContext
{
    public class DbContextScope : IDbContextScope
    {
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            // and see if their first-level cache (i.e. their ChangeTraker) contains the provided entities. 
            ...
            foreach (var contextInCurrentScope in _dbContexts.InitializedDbContexts.GetDbContexts())
            {
                ...
                foreach (var toRefresh in entityList)
                {
                    ...
                    // already been loaded in the parent DbContext's first-level cache (the ChangeTracker).
#pragma warning disable EF1001 // Internal EF Core API usage.
                    var stateInCurrentScope = contextInCurrentScope.GetDependencies().StateManager.TryGetEntry(toRefresh);
                    if (stateInCurrentScope != null)
                    {
                        ...
                        var stateInParentScope = correspondingParentContext.GetDependencies().StateManager.TryGetEntry(key, keyValues);
...
#pragma warning disable EF1001 // Internal EF Core API usage.
                    var stateInCurrentScope =
                        contextInCurrentScope.GetDependencies().StateManager.TryGetEntry(toRefresh);
                    if (stateInCurrentScope != null)
                    {
                        var entityType = stateInCurrentScope.Entity.GetType();
                        var key = stateInCurrentScope.EntityType.FindPrimaryKey();
                        var keyValues = key.Properties
                            ...
                            .Select(s => entityType.GetProperty(s.Name).GetValue(stateInCurrentScope.Entity))
                            .ToArray();
                        var stateInParentScope = correspondingParentContext.GetDependencies().StateManager
...
" + Environment.StackTrace;
        ...
        /*
         * *  NOTE from Javier Ros (aka tolemaC or JRos):
         * * CallContext used by Mehdi El Gueddari not exists in .Net Standard.
         * * For this port I have created a static class called AdaptedCallContextToNetStandard
         * * which use internally AsyncLocal<T> that is for the same porpoise.
         * * I should be change CallContext use by AsyncLocal<T> but I've decided 
         * * to keep the original code as faithful as possible.
         */
