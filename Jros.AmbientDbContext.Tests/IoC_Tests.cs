...
namespace Jros.AmbientDbContext.Tests
{
    public class IoCFixture : IDisposable
    {
        public class DbContextFactory : IDbContextFactory
        {
            ...
            {
                return _sp.GetService<TDbContext>();// new UserManagementDbContext(options) as TDbContext;
            }
        }
        ...
        public IoCFixture()
        {
            ...
            _conn.Open();
            sc.AddSingleton(_conn);

            sc.AddTransient<IUserManagementDbContext>(sp =>
            {
                var options = new DbContextOptionsBuilder<UserManagementDbContext>()
                    .UseSqlite(sp.GetService<SqliteConnection>()) // Set the connection explicitly, so it won't be closed automatically by EF
                    .Options;
                return new UserManagementDbContext(options);
            });
            sc.AddTransient(sp => (UserManagementDbContext) sp.GetService<IUserManagementDbContext>());
            ...
            sc.AddTransient<IDbContextFactory, DbContextFactory>();
